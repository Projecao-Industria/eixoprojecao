...
export default function DevolucaoLoteDialog({ open, onOpenChange }: Props) {
  ...
  async function fetchEntregas() {
    // Fetch all active (unreturned) deliveries
    ...
    // Get unique bem_ids to fetch bem details
    ...
    const rows: EntregaRow[] = data.map(e => {
      ...
      return {
        ...
        usuario: bem?.usuario || "",
        nfe: bem?.nfe || "",
        setor_nome: (bem as any)?.setores?.nome || "",
  ...
  async function handleDevolver() {
    ...
    // Validate all selected items belong to the same gerente
    ...
    // Update all selected entregas with data_devolucao
  ...
  function generatePDF(items: EntregaRow[], dataDevolucao: string) {
    ...
    doc.save(`devolucao_lote_${dataDevolucao}.pdf`);
  }
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">
                        Nenhum item pendente de devolução para este gerente.
                      </td>
                    </tr>
                  )}
                </tbody>
